...
public class JSUml2Story {
    ...
    ArrayList<TraceObject> fnStack = new ArrayList<TraceObject>();
    ArrayList<TraceObject> eventStack = new ArrayList<TraceObject>();
    ...
    public JSUml2Story(PrintStream fileForAllEpisodes, Episode e) throws IOException {
        ...
        // output = new
        // PrintStream(outputFolder+"sequence_diagrams/"+e.getSource().getCounter()+".js");
        ...
        for (TraceObject to : functionTraceObjects) {
            if (to.getClass().toString().contains("DOMMutationTrace")) {
                ...
                System.out.println("var " + getDiagramIdentifier(to)
                        + " = new DOMMutationTrace(false);");
                System.out.println(getDiagramIdentifier(to) + ".setMutationObject("
                        + dmto.getMutationAsJSON() + ");");
                System.out.println("episode" + episodeSource.getCounter() + ".addMutations("
                ...
                System.out.println("var " + getDiagramIdentifier(to)
                        + " = new DOMElementValueTrace(false);");
                System.out.println(getDiagramIdentifier(to) + ".setValueChangeObject("
                ...
                if (to.getClass().toString().contains("XMLHttpRequestSend")) {
                    ...
                    System.out.println(getDiagramIdentifier(to) + ".setMessage('"
                            + xhtto.getMessage() + "');");
                    ...
                    System.out.println(getDiagramIdentifier(to) + ".setCallbackFunction('"
                            + xhtto.getCallbackFunction() + "');");
                    System.out.println(getDiagramIdentifier(to) + ".setResponse('"
                            + xhtto.getResponse() + "');");
                }
                continue;
            } else {
            ...
            if (to.getClass().toString().contains("FunctionEnter")) {
                ...
                System.out.println("var " + getDiagramIdentifier(to) + " = new FunctionTrace('"
                        + feto.getTargetFunction() + "(" + feto.getArgsLabels() + ")');");
                System.out.println(getDiagramIdentifier(to) + ".setFileName('"
                        + feto.getScopeName() + "');");
                ...
                System.out.println(getDiagramIdentifier(to) + ".setUrl('" + xhtto.getUrl()
                        + "');");
                System.out.println(getDiagramIdentifier(to) + ".setMethodType('"
                        + xhtto.getMethodType() + "');");
                ...
                System.out.println("var " + getDiagramIdentifier(to)
                        + " = new DOMEventTrace(false);");
                System.out.println(getDiagramIdentifier(to) + ".setEventType('"
                        + deto.getEventType() + "');");
                System.out.println(getDiagramIdentifier(to) + ".setEventHandler('"
                        + deto.getEventHandler() + "');");
                System.out.println(getDiagramIdentifier(to) + ".setTargetElement("
                        + deto.getTargetElement() + ");");
            }
            // System.out.println(getDiagramIdentifier(to)+".createDiagramObject("+initialX+", "+initialY+");");
            System.out.println(getDiagramIdentifier(to) + ".x_pos = " + initialX + ";");
            System.out.println(getDiagramIdentifier(to) + ".y_pos = " + initialY + ";");
            System.out.println("episode" + episodeSource.getCounter() + ".addComponent("
                    + getDiagramIdentifier(to) + ");");
            System.out.println("");
            initialX += 200;
        }
    ...
    private void addTimeoutInfo(TraceObject to) {
        ...
        if (to.getClass().toString().contains("TimeoutSet")) {
            ...
            TimeoutSet tsto = (TimeoutSet) to;
            System.out
            .println("comment(" + getDiagramIdentifier(to) + ",C, up, wid 1.5 ht .7 \\");
            ...
            System.out
            .println("comment(" + getDiagramIdentifier(to) + ",C, up, wid 1.5 ht .6 \\");
            System.out.println("\"Timing ID: " + ttto.getId() + "\" \\");
            System.out.println("\"Callback: " + ttto.getCallbackFunction() + "\")");
        }
    }
    ...
    public void createMessages() {
        ...
        for (int j = 0; j < functionTraceObjects.size(); j++) {
            ...
            if (functionTraceObjects.get(j).getClass().toString()
                    .contains("DOMElementValueTrace")
                    || functionTraceObjects.get(j).getClass().toString()
                    .contains("DOMMutationTrace")) {
        ...
        for (int i = 0; i < functionTraceObjects.size(); i++) {
            ...
            if (to.getClass().toString().contains("FunctionEnter")) {
                ...
                initialY += 60;
                if (functionTraceObjects.get(i - 1).getClass().toString()
                        .contains("DOMEventTrace")
                        ||
                        functionTraceObjects.get(i - 1).getClass().toString()
                        .contains("TimeoutCallback")
                        ||
                        functionTraceObjects.get(i - 1).getClass().toString()
                        .contains("XMLHttpRequestResponse")
                        ||
                        functionTraceObjects.get(i - 1).getClass().toString()
                    ...
                    //functionEnterMessage(fnStack.get(fnStack.size()-1), to, initialY);
                } else if (fnStack.size() > 0 /*&& fnStack.get(fnStack.size()-1).getClass().toString().contains("FunctionEnter")*/) {
                    // Set new function as active

                    functionEnterMessage(fnStack.get(fnStack.size()-1), to, initialY);

                } else if (eventStack.size() > 0 && to.getTimeStamp() - eventStack.get(eventStack.size()-1).getTimeStamp() < 200) {
    ...
    private void TimeoutSetMessage(TraceObject before, TraceObject to, int y) {
        if (before.getClass().toString().contains("FunctionCall")) {
            ...
            if (fcto.getTargetFunction().contains("setTimeout")) {
                ...
                System.out.println("var message_" + getDiagramIdentifier(to) + "_" + y
                        + " = new callMessage({a : "
                        + functionHeirarchy.get(functionHeirarchy.size() - 1) + ", " +
                        "b : " + getDiagramIdentifier(to) + ", " +
                        "y : " + y + "});");
                System.out.println("message_" + getDiagramIdentifier(to) + "_" + y
                        + ".setName(\"setTimeout\");");
    ...
    private void XHROpenMessage(TraceObject to, int y) {
        ...
        System.out.println("message_" + getDiagramIdentifier(to) + "_" + y
                + ".setName(\"open\");");
        // System.out.println("message_"+getDiagramIdentifier(to)+"_"+y+".notifyChange()");
        System.out.println("episode" + episodeSource.getCounter() + ".addMessage(message_"
                + getDiagramIdentifier(to) + "_" + y + ");");
        pop();
    }
    ...
    private void functionEnterMessage(TraceObject from, TraceObject to, int y) {
        ...
        if (feto.getArgs() != null) {
            System.out.println("var message_" + getDiagramIdentifier(to) + "_" + y
                    + " = new callMessage({a : " + fromID + ", " +
                    "b : " + getDiagramIdentifier(to) + ", " +
                    "y : " + y + "});");
            System.out.println("message_" + getDiagramIdentifier(to) + "_" + y
                    + ".setName(\"args: " + feto.getArgsString() + "\");");
            // System.out.println("message_"+getDiagramIdentifier(to)+"_"+y+".notifyChange()");
            System.out.println("episode" + episodeSource.getCounter() + ".addMessage(message_"
                    + getDiagramIdentifier(to) + "_" + y + ");");
            ...
            // No arguments
            System.out.println("episode" + episodeSource.getCounter()
                    + ".addMessage(new callMessage({a : " + fromID + ", " +
                    "b : " + getDiagramIdentifier(to) + ", " +
                    "y : " + y + "}));");
            ;
        }
